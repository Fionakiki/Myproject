...
<html>
    ...
    <body>
        ...
        $con=mysql_connect("localhost","root","oneinstack");//连接mysql 数据库，账户名root ，密码root
        if (!$con) {
            die('数据库连接失败'.$mysql_error());
        }
        mysql_select_db("wwcuser",$con);//use wwcuser数据库；
        $dbusername=null;
        $dbpassword=null;
        $result=mysql_query("select * from wwcuser where username ='{$username}' and isdelete =0;");//查出对应用户名的信息，isdelete表示在数据库已被删除的内容
        ...
        <?php
            ...
            <script type="text/javascript">
                alert("登入成功");
                window.location.href="../html/index_edit2.html";
            </script>
            <?php
            }
        }
